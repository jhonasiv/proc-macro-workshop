...
enum GenericVariant<'a> {
    Associative(&'a syn::punctuated::Punctuated<syn::PathSegment, syn::Token![::]>),
    SameGeneric,
    Phantom,
    DifferentFromGeneric,
}

impl<'a> GenericVariant<'a> {
    pub fn prioritize(new_variant: Self, old_variant: Self) -> Self {
        match (&new_variant, &old_variant) {
            ...
            (_, Self::Associative(a)) => Self::Associative(a),
            (Self::Associative(a), _) => Self::Associative(a),
...
fn compute_trait_bounds<'a>(
    mut generics: syn::Generics,
    types: &'a ::std::vec::Vec<syn::Type>,
    ...
    for param in &mut generics.params {
        match param {
            syn::GenericParam::Type(ref mut type_param) => {
                ...
                if let Some(GenericVariant::Associative(at)) =
                    generic_variant.iter().find(|v| match v {
                        GenericVariant::Associative(_) => true,
                        _ => false,
                    })
                {
                    let parsed_ty: syn::Type = syn::parse2(at.to_token_stream())?;
...
fn compute_generic_variant<'a>(
    types: &'a ::std::vec::Vec<syn::Type>,
    generic_ident: &'a syn::Ident,
) -> ::std::vec::Vec<GenericVariant<'a>> {
    types
        .iter()
        .map(|ty| match ty {
            syn::Type::Path(syn::TypePath {
                ...
                |variant, segment| {
                    ...
                    let inner_variant = if segments.len() == 3
                        && segments.clone().into_pairs().fold(false, |res, pair| {
                            let match_structure = match pair {
                        ...
                        GenericVariant::Associative(segments)
...
fn recursive_search_over_arguments_for_type<'a>(
        ...
        inner_args.iter().fold(
            ...
            |arg_variant, arg| match arg {
                syn::GenericArgument::Type(syn::Type::Path(syn::TypePath {
                    ...
                    // Associative type from this generic
                    let seg_variant = if segments.len() == 2
                        && segments.clone().into_pairs().fold(true, |res, pair| {
                            let match_associative = match pair {
                                syn::punctuated::Pair::Punctuated(seg, _) => {
                                    &seg.ident == generic_ident
                                },
                                syn::punctuated::Pair::End(_) => true,
                            };
                            res && match_associative
                        }) {
                        GenericVariant::Associative(segments)
                    } else {
                        segments.iter().fold(
